...
kcpf()
{
  FINALIZERS=$(kubectl get --ignore-not-found=true -o jsonpath="{.metadata.finalizers}" "$@")
...
for CRD in $(kubectl get crd -o name | grep cattle\.io | grep -v helm\.cattle\.io | grep -v k3s\.cattle\.io | grep -v fleet); do
  kcrd "$CRD"
done
...
kubectl delete --all pods --namespace=cattle-system
kubectl -n cattle-system delete secret bootstrap-secret --ignore-not-found=true
...
# Delete apiservice
for APISERVICE in $(kubectl  get apiservice -o name | grep cattle | grep -v k3s\.cattle\.io | grep -v helm\.cattle\.io); do
  kcd "$APISERVICE"
done
...
for NS in $FLEET_NAMESPACES $CATTLE_NAMESPACES; do
...
# kubectl get validatingwebhookconfigurations -o name | while read -r CRB; do
#   kcd "$CRB"
# done

# kubectl get mutatingwebhookconfigurations -o name | while read -r CRB; do
#   kcd "$CRB"
# done
